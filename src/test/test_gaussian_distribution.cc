...
template<typename T>
bool DoWork(int argc, char *argv[], const std::string &capacity,
...
int main(int argc, char *argv[]) {
  ...
  if (gen_flag) {
    CALL_FUNC_T(suc, t_str, DoWork,
                (argc, argv, capacity, data_path, ext),
                {
...
bool DoWork(int argc, char *argv[], const std::string &capacity,
  ...
  gpusort::GaussianChecker<T> checker(comm);
...
bool DoWork(int argc, char *argv[], gpusort::Algorithm algo,
            bool reverse, unsigned int kway, bool is_using_gpu,
            const std::string &data_path, const std::string &ext) {
  return gpusort::Utils::Sort<T, gpusort::GaussianChecker>(
