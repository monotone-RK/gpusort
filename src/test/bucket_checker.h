#ifndef GPUSORT_TEST_BUCKET_CHECKER_H_
#define GPUSORT_TEST_BUCKET_CHECKER_H_
...
template<typename T>
class BucketChecker : public Utils::DataChecker<T> {
 public:
  BucketChecker() : Utils::DataChecker<T>() {
  }

  BucketChecker(MpiComm comm) : Utils::DataChecker<T>(comm) {
  }

  bool Check(const std::vector<T> &list,
             bool is_increasing) throw(MpiException) {
    if (MpiUtils::MpiCommRank(this->comm_) == 0)
      std::cout << "Validating sorted sequence by BucketChecker."
                << std::endl;
...
#endif  // GPUSORT_TEST_BUCKET_CHECKER_H_
