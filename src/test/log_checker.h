#ifndef GPUSORT_TEST_LOG_CHECKER_H_
#define GPUSORT_TEST_LOG_CHECKER_H_
...
template<typename T>
class LogChecker : public Utils::DataChecker<T> {
 public:
  LogChecker() : Utils::DataChecker<T>() {
  }

  LogChecker(MpiComm comm) : Utils::DataChecker<T>(comm) {
  }
  ...
  bool Check(const std::vector<T> &list,
    ...
    if (myrank == 0) std::cout << "Validating sorted sequence by LogChecker "
  ...
  bool Generate(int64_t nelem, std::vector<T> *list) throw(MpiException) {
    ...
    std::string msg = "Generating a sequence using Log function with: " +
                      ...
                      ((shuffle)? "true" : "false") + ".\n";
    std::cout << msg;
    Utils::GenDataUsingLogFunc(begin, nelem, shuffle, list);
    return true;
  }
...
#endif  // GPUSORT_TEST_LOG_CHECKER_H_
